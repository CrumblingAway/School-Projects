# -*- coding: utf-8 -*-
"""Копия блокнота "IMPR Ex5.ipynb"

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1-D4ud0vx1FfTIDU-NDq4iyiLEWPMEu9d
...
cs_id = "kublakhan" #@param {type:"string"}
...
def load_dataset(filenames, batch_size, corruption_func, crop_size):
    ...
    while True:
        ...
        paths = np.random.choice(filenames, batch_size, True)

        for idx, path in enumerate(paths):
            # Check cache
            if path not in im_cache: im_cache[path] = read_image(path, 1)
            src_image = im_cache[path]

            # Crop 3*crop_size
            large_i, large_j = np.random.randint(src_image.shape[0] - 3 * crop_size[0]), \
                               np.random.randint(src_image.shape[1] - 3 * crop_size[1])
            src_large_crop = src_image[large_i: (large_i + 3 * crop_size[0]),   
                                       large_j: (large_j + 3 * crop_size[1])]

            # Corrupt and crop
            corr_image = corruption_func(src_large_crop)    
            small_i, small_j = np.random.randint(src_large_crop.shape[0] - crop_size[0]), \
                               np.random.randint(src_large_crop.shape[1] - crop_size[1])
            src_patch = src_large_crop[small_i: (small_i + crop_size[0]), small_j: (small_j + crop_size[1])] - 0.5
            target_patch = corr_image[small_i: (small_i + crop_size[0]), small_j: (small_j + crop_size[1])] - 0.5
            source_batch[idx] = target_patch[..., np.newaxis]
            target_batch[idx] = src_patch[..., np.newaxis]
...
def build_nn_model(height, width, num_channels, num_res_blocks):
    ...
    # Add and return model
    b = Conv2D(1, (3, 3), padding='same')(b)
    x = Add()([a, b])
    return Model(inputs=a, outputs=x)
...
def train_model(model, images, corruption_func, batch_size, steps_per_epoch, num_epochs, num_valid_samples):
    ...
    """
    x_train, x_validation = train_test_split(images, train_size=0.8)
    crop_size = (model.input_shape[1], model.input_shape[2])
    train_set = load_dataset(x_train, batch_size, corruption_func, crop_size)
    validation_set = load_dataset(x_validation, batch_size, corruption_func, crop_size)
...
def restore_image(corrupted_image, base_model):
    ...
    corrupted_image = (corrupted_image - 0.5).reshape((1, h, w, 1))
    result = new_model.predict(corrupted_image)[0].astype(np.float64)
    result = result.reshape((h, w)) + 0.5
...
def add_gaussian_noise(image, min_sigma, max_sigma):
    ...
    im_noisy = np.round((image + noise) * 255) / 255
    return np.clip(im_noisy, 0, 1)
...
def learn_denoising_model(denoise_num_res_blocks, quick_mode=False):
    ...
    """
    slow_args = {'batch_size': 100, 'steps_per_epoch': 100, 'num_epochs': 10, 'num_valid_samples': 1000}
    quick_args = {'batch_size': 10, 'steps_per_epoch': 3, 'num_epochs': 2, 'num_valid_samples': 30}
    args = quick_args if quick_mode else slow_args
    
    filenames = images_for_denoising()
    model = build_nn_model(24, 24, 48, denoise_num_res_blocks)
    train_model(model, filenames, lambda x: add_gaussian_noise(x, 0, 0.2), 
                args['batch_size'], args['steps_per_epoch'], args['num_epochs'], args['num_valid_samples'])
    return model
...
def random_motion_blur(image, list_of_kernel_sizes):
    ...
    result = np.round(add_motion_blur(image, kernel_size, angle) * 255) / 255
    return np.clip(result, 0, 1)
...
def learn_deblurring_model(deblur_num_res_blocks, quick_mode=False):
    ...
    train_model(model, filenames, lambda x: random_motion_blur(x, [7]), 
                args['batch_size'], args['steps_per_epoch'], args['num_epochs'], args['num_valid_samples'])
    return model
...
def super_resolution_corruption(image):
    ...
    """
    zoom_factor = 1 / np.random.randint(2, 5)
    im_zoom = zoom(image, zoom_factor)
    inverse_factor = np.array(image.shape) / np.array(im_zoom.shape)
    return zoom(im_zoom, inverse_factor, order=0)
...
def learn_super_resolution_model(super_resolution_num_res_blocks, quick_mode=False):
    ...
    train_model(model, filenames, lambda x: super_resolution_corruption(x), 
                args['batch_size'], args['steps_per_epoch'], args['num_epochs'], args['num_valid_samples'])
    return model
...
Answer1 = "The SR corruption function simply takes an image and zooms out then zooms in. We set the order (i.e. the degree of the polynomial for interpolation) to be 0 in order to achieve a very basic interpolation, because if we raise the degree we might achieve a more accurate (and less blurry) depiction of the original image. . " #@param {type:"string"}
...
Answer2 = "I picked the the same values as in the deblurring model. " #@param {type:"string"}
...
def effect_of_depth():
    ...
    """
    models = [
              {'model': learn_denoising_model, 'title': 'Denoising validation MSE', 'path': './depth_plot_denoise.png'},
              {'model': learn_deblurring_model, 'title': 'Deblurring validation MSE', 'path': './depth_plot_deblur.png'},
            #   {'model': learn_super_resolution_model, 'title': 'Super resolution validation MSE', 'path': './depth_plot_super_resolution.png'}
              ]

    for model_struct in models:
        for i in range(1, 6):
            # print(model_struct['title'], i)
            model = model_struct['model'](i)
            model.save("saveModels/{}_{}blocks.h5".format(str(model_struct['path'][13:-4]), str(i)))
            plt.plot(model.history.history['val_loss'])
        plt.title(model_struct['title'])
        plt.xlabel('Epoch')
        plt.ylabel('Validation MSE')
        plt.legend(['{} res block'.format(i) for i in range(1, 6)], loc='upper right')
        plt.savefig(model_struct['path'])
        plt.show()
...
Answer3 = "In both denoising and deblurring the validation error decreased the more we increased the amount of residual blocks. The lowering of the error is reflected in the quality of the images which also improves the more we increase the amount of residual blocks. In contrast when we look at SR the best result is achieved for 3 residual blocks (reflected in the quality of the images). The results in SR for 4 and 5 residual blocks are slightly worse than that of 3." #@param {type:"string"}
